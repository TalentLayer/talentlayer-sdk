...
export function getCompletionScores(user: IUser): ICompletionScores {
  let score = 0;
  let totalPoint = 0;
  ...
  Object.entries(pointsByWeb3mailFields).forEach((pointsByField) => {
    ...
    if (
      ...
      user.description.web3mailPreferences[
        key as keyof IWeb3mailPreferences
      ] !== null
    ) {
      score += value;
  ...
  return {
    ...
    web3mail: {
      percentage: Math.round(
        (userWeb3mailScore * 100) / userWeb3mailTotalPoint
      ),
