...
export class Escrow {
    ...
    public async approve(
        ...
        const protocolAndPlatformsFeesResponse = await this.graphQlClient.get(
            getProtocolAndPlatformsFees(
                proposal.service.platform.id,
                proposal.platform.id,
            ),
        );
        ...
        const approvalAmount = calculateApprovalAmount(
            proposal.rateAmount,
            protocolAndPlatformsFeesResponse.data.servicePlatform
                .originServiceFeeRate,
            protocolAndPlatformsFeesResponse.data.proposalPlatform
        ...
        if (proposal.rateToken.address === RateToken.NATIVE) {
            ...
            // @ts-ignore
            const allowance: bigint = await erc20.checkAllowance(
                proposal.rateToken.address,
            );

            console.log(
                "SDK: fetched allowance",
                allowance,
                allowance < BigInt(proposal.rateAmount),
            );

            if (allowance < approvalAmount) {
                console.log(
                    "SDK: approvalAmount less than allowance. Now requesting allowance",
                );
                ...
                try {
                    ...
                    // @ts-ignore
                    const approvalTransactionReceipt = await this.viemClient.publicClient.waitForTransactionReceipt({
                        hash: approvalTransaction,
                    });

                    console.log(
                        "SDK: approvalTransactionReceipt",
            ...
            tx = await this.viemClient.writeContract(
                "talentLayerEscrow",
                "createTransaction",
                [serviceId, sellerId, metaEvidenceCid, cid],
            );
        }
    ...
    public async release(
        ...
        const tx = await this.viemClient.writeContract(
            "talentLayerEscrow",
            "release",
            [userId, parseInt(transactionId), amount.toString()],
        );
